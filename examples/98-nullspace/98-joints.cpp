...
int main(int argc, char** argv) {
	...
	chai3d::cShapeBox* box = new chai3d::cShapeBox(ellipsoid_radii(0) * 2, ellipsoid_radii(1) * 2, 0.0001);
	// chai3d::cShapeBox* box = new chai3d::cShapeBox(ellipsoid_radii(0) * 2, ellipsoid_radii(1) * 2, ellipsoid_radii(1));
	...
	box->setTransparencyLevel(0.3);
	graphics->_world->addChild(box);

    chai3d::cMesh* right_sphere = new chai3d::cMesh();
    chai3d::cCreateSphere(right_sphere, ellipsoid_radii(1));
	// chai3d::cShapeSphere* right_sphere = new chai3d::cShapeSphere(ellipsoid_radii(1));
    
    for (int i = 0; i < right_sphere->getNumVertices(); ++i) {
        auto vert = right_sphere->m_vertices->getLocalPos(i);
        if (vert(0) < 0) {
            vert(0) = 0;
            vert(2) = 0;
            right_sphere->m_vertices->setLocalPos(i, vert);
        } else {
            vert(2) = 0;
            right_sphere->m_vertices->setLocalPos(i, vert);
        }
	...
	right_sphere->setTransparencyLevel(0.3);
    // right_sphere->setNormalsProperties(0, chai3d::cColorf(1, 1, 1));

	graphics->_world->addChild(right_sphere);
    
    chai3d::cMesh* left_sphere = new chai3d::cMesh();
    chai3d::cCreateSphere(left_sphere, ellipsoid_radii(1));
	// chai3d::cShapeSphere* left_sphere = new chai3d::cShapeSphere(ellipsoid_radii(1));

    for (int i = 0; i < left_sphere->getNumVertices(); ++i) {
        auto vert = left_sphere->m_vertices->getLocalPos(i);
        if (vert(0) > 0) {
            vert(0) = 0;
            vert(2) = 0;
            left_sphere->m_vertices->setLocalPos(i, vert);
        } else {
            vert(2) = 0;
            left_sphere->m_vertices->setLocalPos(i, vert);
        }
	...
	left_sphere->setTransparencyLevel(0.3);
	...
	outer_box->setTransparencyLevel(0.3);
	graphics->_world->addChild(outer_box);

    chai3d::cMesh* outer_right_sphere = new chai3d::cMesh();
    chai3d::cCreateSphere(outer_right_sphere, ellipsoid_radii(1) + 0.3);
	// chai3d::cShapeSphere* outer_right_sphere = new chai3d::cShapeSphere(ellipsoid_radii(1) + 0.3);

    for (int i = 0; i < outer_right_sphere->getNumVertices(); ++i) {
        auto vert = outer_right_sphere->m_vertices->getLocalPos(i);
        if (vert(0) < 0) {
            vert(0) = 0;
            vert(2) = 0;
            outer_right_sphere->m_vertices->setLocalPos(i, vert);
        } else {
            vert(2) = 0;
            outer_right_sphere->m_vertices->setLocalPos(i, vert);
        }
	...
	outer_right_sphere->setTransparencyLevel(0.3);
    ...
    chai3d::cMesh* outer_left_sphere = new chai3d::cMesh();
    chai3d::cCreateSphere(outer_left_sphere, ellipsoid_radii(1) + 0.3);

    for (int i = 0; i < outer_left_sphere->getNumVertices(); ++i) {
        auto vert = outer_left_sphere->m_vertices->getLocalPos(i);
        if (vert(0) > 0) {
            vert(0) = 0;
            vert(2) = 0;
            outer_left_sphere->m_vertices->setLocalPos(i, vert);
        } else {
            vert(2) = 0;
            outer_left_sphere->m_vertices->setLocalPos(i, vert);
        }
	...
	outer_left_sphere->setTransparencyLevel(0.3);
	...
	nullspace_force_line->m_colorPointA.set(1, 0, 0);  // nullspace line green 
	...
	chai3d::cShapeCylinder* goal_sphere = new chai3d::cShapeCylinder(0.1, 0.1, 0.04);
	goal_sphere->setLocalPos(chai3d::cVector3d(3, 2, -0.02));
	goal_sphere->m_material->setRed();
	// outer_left_sphere->setTransparencyLevel(0.3);
	graphics->_world->addChild(goal_sphere);

    // std::cout << "Right sphere local position: \n" << right_sphere->getLocalPos() << "\n";
    // throw runtime_error("");
	...
	// obstacle->setTransparencyLevel(0.3);
	...
	// outer_obstacle->setTransparencyLevel(0.3);
...
void control(shared_ptr<Sai2Model::Sai2Model> robot,
    ...
    // Capsule collision
    Vector2d capsule_a = ellipsoid_center - Vector2d(ellipsoid_radii(0), 0) - 1 * Vector2d(ellipsoid_radii(1), 0);
    Vector2d capsule_b = ellipsoid_center + Vector2d(ellipsoid_radii(0), 0) + 1 * Vector2d(ellipsoid_radii(1), 0);
    double capsule_radius = ellipsoid_radii(1);
    ...
    Vector2d x_goal = Vector2d(3, 2);
    // Vector2d x_delta = Vector2d(1, 2);
    // Vector2d x_delta = Vector2d(2, 1);
    // Vector2d x_delta = Vector2d(1.25, 1) + x_init;
	bool first_loop = true;
	bool first_entry = true;
	// double zone_width = 0.3 + 0.1 * 0;
    double zone_width = 0.3;
	double outer_zone_width = zone_width;

	// joint_task->setGoalPosition(joint_task->getCurrentPosition() + x_delta);
	joint_task->setGoalPosition(x_goal);
    ...
    Vector2d position = robot->q().head(2);

	// create logger
	Sai2Common::Logger logger("apf", false);
    logger.addToLog(position, "position");
	...
	while (fSimulationRunning) {
		...
		if (distance > outer_zone_width) {
            ...
            if (!joint_task->goalPositionReached(1e-3)) {
                ...
                nullspace_force_line->m_pointB = chai3d::cVector3d(ee_pos) + 1 * joint_task_torques.head(2).norm() * chai3d::cVector3d(task_forces_normalized(0), task_forces_normalized(1), 0);
            } else {
			    nullspace_force_line->m_pointB = chai3d::cVector3d(ee_pos) + 0 * chai3d::cVector3d(task_forces_normalized(0), task_forces_normalized(1), 0);
                // nullspace_force_line->m_pointB = chai3d::cVector3d(ee_pos) + 0.5 * joint_task_torques.head(2).norm() * chai3d::cVector3d(task_forces_normalized(0), task_forces_normalized(1), 0);
            }
		// } else if (!first_entry) {
        } else if (true) {
        // if (distance < zone_width) {
		// if (!first_entry) {
			...
			// force_line->m_pointB = chai3d::cVector3d(ee_pos) + 0.5 * chai3d::cVector3d(task_forces_without_nullspace(0), \
			// 																				task_forces_without_nullspace(1), 0);
			force_line->m_pointB = chai3d::cVector3d(ee_pos) + 0.5 * chai3d::cVector3d(apf_force(0), apf_force(1), 0);
			nullspace_force_line->m_pointA = chai3d::cVector3d(ee_pos);
			auto task_forces_normalized = joint_task_torques.normalized();
			...
			nullspace_force_line->m_pointA = chai3d::cVector3d(ee_pos);
			auto task_forces_normalized = joint_task_torques.head(2).normalized();
            nullspace_force_line->m_pointB = chai3d::cVector3d(ee_pos) + 0.5 * joint_task_torques.head(2).norm() * chai3d::cVector3d(task_forces_normalized(0), task_forces_normalized(1), 0);
			// nullspace_force_line->m_pointB = chai3d::cVector3d(ee_pos) + 0.5 * chai3d::cVector3d(task_forces_normalized(0), task_forces_normalized(1), 0);
		}
		...
		//------ compute the final torques
		{
			...
			control_torques = joint_task_torques + 1 * apf_force.head(2);
