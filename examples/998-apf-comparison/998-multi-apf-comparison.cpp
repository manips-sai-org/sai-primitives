...
const string yaml_fname = "../../../examples/998-apf-comparison/multi_params.yaml";
...
Vector2d RHO_0;
Vector2d RHO_1;
Vector2d APF_A, APF_B, SF;
double VEL, MIN_VEL, VEL_SF;
double KV, T_WAIT;
Vector2d DISTANCE = 100 * Vector2d::Ones();
Vector2d APF0_CENTER = Vector2d::Zero();
Vector2d APF1_CENTER = Vector2d::Zero();
std::vector<Vector2d> CLOSEST_POINT = {2, Vector2d::Zero()};
double MASS = 1;
bool NULLSPACE_FLAG = false;
bool EXIT_FLAG = false;
bool SLOWDOWN_FLAG = false;
std::vector<Vector2d> CONSTRAINT_VECTOR = {2, Vector2d::Zero()};
    ...
    Vector2d center = Vector2d::Zero())
{
    ...
    for (int i = 0; i < numPoints; ++i)
    {
        ...
        originalPoints.push_back(cVector3d(x + center(0), y + center(1), 0));
        ...
        Eigen::Vector2d offset = Eigen::Vector2d(x + center(0), y + center(1)) + radius * normal;
...
double distanceToSuperellipse(double x0, double y0, double a, double b, double x = 0, double y = 0) {
    Eigen::Vector2d closest = projectToSuperellipse(x0 - x, y0 - y, a, b);
    return (closest - Eigen::Vector2d(x0, y0) + Vector2d(x, y)).norm();
    // return (closest - Vector2d(x0, y0)).norm();
}

bool isInsideSuperellipse(double x0, double y0, double a, double b, double x = 0, double y = 0) {
    double val = std::pow(std::abs((x0 - x) / a), 8) + std::pow(std::abs((y0 - y) / b), 8);
    return val <= 1.0;
}
...
Vector2d computeApfForce(const double a, 
	...
	auto closest_point = closestPointToSuperellipse(p - center, a, b);

	CLOSEST_POINT[idx] = closest_point + center;
	// std::cout << "closest point: " << closest_point.transpose() << "\n";
	// std::cout << "current robot: " << p.transpose() << "\n";
	CONSTRAINT_VECTOR[idx] = (p - CLOSEST_POINT[idx].head(2)).normalized();
	...
	double distance = (p - CLOSEST_POINT[idx].head(2)).norm();
	DISTANCE(idx) = distance;

	// std::cout << "current distance: " << distance << "\n";

	if (distance > rho_0) {
		// CONSTRAINT_VECTOR[idx].setZero();
		// if (FIRST_ENTRY) {
			// FIRST_ENTRY = false;
		// }
		INSIDE_APF = false;
		...
		Vector2d force = eta * ((1 / distance) - (1 / rho_0)) * (1 / (distance * distance)) * CONSTRAINT_VECTOR[idx].normalized();
    ...
    Vector2d center = Vector2d::Zero())
{
    ...
    int centerTopIdx = mesh->newVertex(cVector3d(0.0, 0.0, z_top) + cVector3d(center(0), center(1), 0));
    int centerBotIdx = mesh->newVertex(cVector3d(0.0, 0.0, z_bot) + cVector3d(center(0), center(1), 0));
    mesh->m_vertices->setNormal(centerTopIdx, cVector3d(0, 0, 1) + cVector3d(center(0), center(1), 0));
    mesh->m_vertices->setNormal(centerBotIdx, cVector3d(0, 0, -1) + cVector3d(center(0), center(1), 0));
    ...
    for (int i = 0; i < resolution; ++i)
    {
        ...
        int idxTop = mesh->newVertex(cVector3d(x, y, z_top) + cVector3d(center(0), center(1), 0));
        int idxBot = mesh->newVertex(cVector3d(x, y, z_bot) + cVector3d(center(0), center(1), 0));

        mesh->m_vertices->setNormal(idxTop, cVector3d(0, 0, 1) + cVector3d(center(0), center(1), 0));
        mesh->m_vertices->setNormal(idxBot, cVector3d(0, 0, -1) + cVector3d(center(0), center(1), 0));
...
chai3d::cShapeLine* second_closest_distance_line;
...
int main(int argc, char** argv) {
	...
	*/
    ...
    for (int i = 0; i < 2; ++i) {
	    ETA(i) = config["object_config"]["eta_" + std::to_string(i)].as<double>();
	    RHO_0(i) = config["object_config"]["rho0_" + std::to_string(i)].as<double>();
	    SF(i) = config["object_config"]["sf_" + std::to_string(i)].as<double>();
        APF_A(i) = config["object_config"]["a_" + std::to_string(i)].as<double>();
	    APF_B(i) = config["object_config"]["b_" + std::to_string(i)].as<double>();
 	    RHO_1(i) = config["object_config"]["rho1_" + std::to_string(i)].as<double>();
    }
    
    APF0_CENTER = Vector2d(config["object_config"]["apf_center_0_x"].as<double>(), config["object_config"]["apf_center_0_y"].as<double>());
    APF1_CENTER = Vector2d(config["object_config"]["apf_center_1_x"].as<double>(), config["object_config"]["apf_center_1_y"].as<double>());
	...
	auto obstacle = createFilled2DSuperellipse_n4(APF_A(0), APF_B(0), resolution, 0 * (0.002 - 0.003), color, 1.0, THICKNESS);
    auto second_obstacle = createFilled2DSuperellipse_n4(APF_A(0), APF_B(0), resolution, 0 * (0.002 - 0.003), color, 1.0, THICKNESS, APF1_CENTER);
    ...
    // obstacle 1
	generateSuperellipseWithOffset(APF_A(0), APF_B(0), RHO_0(0) + 0.001 * 0, numSamples, superellipse, outerBoundary);
    ...
    // obstacle 2
    generateSuperellipseWithOffset(APF_A(1), APF_B(1), RHO_0(1) + 0.001 * 0, numSamples, superellipse, outerBoundary, APF1_CENTER);
	...
	graphics->_world->addChild(second_obstacle);
	...
	if (SLOWDOWN_FLAG) {
		...
		generateSuperellipseWithOffset(APF_A(0), APF_B(0), RHO_1(0), numSamples, outerBoundary, velBoundary);
		fillSuperellipseBand(graphics->_world,
							 outerBoundary,
							 velBoundary,
							 color,
        ...
        generateSuperellipseWithOffset(APF_A(1), APF_B(1), RHO_1(1), numSamples, outerBoundary, velBoundary, Vector2d(APF1_CENTER(0), APF1_CENTER(1)));
		fillSuperellipseBand(graphics->_world,
							 outerBoundary,
							 velBoundary,
							 color,
    ...
    // second obstacle
    second_closest_distance_line = new chai3d::cShapeLine();
	color = chai3d::cColorf(0.46, 0.82, 0.44);
	second_closest_distance_line->m_colorPointA = color;
	second_closest_distance_line->m_colorPointB = color;
	second_closest_distance_line->m_pointA = chai3d::cVector3d(100, 0, 0);
	second_closest_distance_line->m_pointB = chai3d::cVector3d(100, 0, 0);
	second_closest_distance_line->setLineWidth(10);
	graphics->_world->addChild(second_closest_distance_line);
	...
	while (graphics->isWindowOpen()) {
		{
			...
			T_object.translation() = Vector3d(CLOSEST_POINT[0](0), CLOSEST_POINT[0](1), 0);
			graphics->updateObjectGraphics("closest_point", T_object);
            T_object.translation() = Vector3d(CLOSEST_POINT[1](0), CLOSEST_POINT[1](1), 0);
			graphics->updateObjectGraphics("closest_point_2", T_object);
			...
			closest_distance_line->m_pointB = chai3d::cVector3d(CLOSEST_POINT[0](0), CLOSEST_POINT[0](1), 0);

            second_closest_distance_line->m_pointA = chai3d::cVector3d(robot->q()(0), robot->q()(1), 0);
			second_closest_distance_line->m_pointB = chai3d::cVector3d(CLOSEST_POINT[1](0), CLOSEST_POINT[1](1), 0);
...
void control(shared_ptr<Sai2Model::Sai2Model> robot,
	...
	while (fSimulationRunning) {
		...
		// velocity saturation check (obstacle 1 and 2)
		if (SLOWDOWN_FLAG) {
			// DISTANCE(0) = distanceToSuperellipse(robot->q()(0), robot->q()(1), VEL_SF * APF_A(0), VEL_SF * APF_B(0), APF0_CENTER(0), APF0_CENTER(1));
			double first_distance_to_outer = distanceToSuperellipse(robot->q()(0), robot->q()(1), APF_A(0), APF_B(0));

            // DISTANCE(1) = distanceToSuperellipse(robot->q()(0), robot->q()(1), VEL_SF * APF_A(1), VEL_SF * APF_B(1), APF1_CENTER(0), APF1_CENTER(1));
			double second_distance_to_outer = distanceToSuperellipse(robot->q()(0), robot->q()(1), APF_A(1), APF_B(1), APF1_CENTER(0), APF1_CENTER(1));

			// if (first_distance_to_outer <= RHO_0(0) || second_distance_to_outer <= RHO_0(1)) {
            if (first_distance_to_outer <= RHO_0(0) || second_distance_to_outer <= RHO_0(1)) {
            // if (first_distance_to_outer <= RHO_0(0)) {
				motion_task->enableVelocitySaturation(MIN_VEL);
			} else if (first_distance_to_outer <= RHO_1(0) || second_distance_to_outer <= RHO_1(1)) {
			// } else if (first_distance_to_outer <= RHO_1(0) ) {
				double first_alpha = 1 - std::clamp((first_distance_to_outer - RHO_0(0)) / (RHO_1(0) - RHO_0(0)), 0.0, 1.0);
				double first_max_vel = getMaxVelFunction(first_alpha, VEL, MIN_VEL);
                
                double second_alpha = 1 - std::clamp((second_distance_to_outer - RHO_0(1)) / (RHO_1(1) - RHO_0(1)), 0.0, 1.0);
				double second_max_vel = getMaxVelFunction(second_alpha, VEL, MIN_VEL);

				motion_task->enableVelocitySaturation(std::min(first_max_vel, second_max_vel));
				// motion_task->enableVelocitySaturation(first_max_vel);
                // motion_task->enableVelocitySaturation(MIN_VEL);
			} else {
				motion_task->enableVelocitySaturation(VEL);
                // motion_task->enableVelocitySaturation(MIN_VEL);
        ...
        std::vector<MatrixXd> N_obs = {2, MatrixXd::Identity(dof, dof)};
		...
		// compute apf torques 
		auto first_apf_torque = computeApfForce(APF_A(0), APF_B(0), robot->q(), Vector2d(0, 0), ETA(0), RHO_0(0), 0, flag_project);  // 2 x 1 force vector 
		auto second_apf_torque = computeApfForce(APF_A(1), APF_B(1), robot->q(), APF1_CENTER, ETA(1), RHO_0(1), 1, flag_project);  // 2 x 1 force vector 
		// std::cout << "apf torque: " << first_apf_torque.transpose() << "\n";
        // if (second_apf_torque(0) != 0) {
            // throw runtime_error("");
        // }
		...
		*/
	    VectorXd first_damping_torques = VectorXd::Zero(2);
	    VectorXd second_damping_torques = VectorXd::Zero(2);
        std::vector<MatrixXd> J_con = {2, Vector2d(0, 0)};
        std::vector<MatrixXd> task_projections = {2, Matrix2d::Identity()};

        // compute distance to inner ellipse 
        // DISTANCE(0) = distanceToSuperellipse(robot->q()(0), robot->q()(1), APF_A(0), APF_B(0), APF0_CENTER(0), APF0_CENTER(1));
        // DISTANCE(1) = distanceToSuperellipse(robot->q()(0), robot->q()(1), APF_A(1), APF_B(1), APF1_CENTER(0), APF1_CENTER(1));

        int smaller_idx = -1;

		if (NULLSPACE_FLAG) {
            // first obstacle
			if (DISTANCE(0) < RHO_0(0)) {
				// std::cout << "constraint vector: " << CONSTRAINT_VECTOR[0].transpose() << "\n";
				// J_con = Vector3d(CONSTRAINT_VECTOR(0), CONSTRAINT_VECTOR(1), 0).transpose() * robot->Jv("link1");
				J_con[0] = CONSTRAINT_VECTOR[0].transpose() * robot->Jv("link1");  // 1 x 2
				Vector2d force_bias = 0 * Vector2d(0, 1);
				first_damping_torques = - KV * J_con[0] * robot->dq();                

                smaller_idx = 0;
            }

            // second obstacle
            if (DISTANCE(1) < RHO_0(1)) {
				// std::cout << "constraint vector: " << CONSTRAINT_VECTOR.transpose() << "\n";
				// J_con = Vector3d(CONSTRAINT_VECTOR(0), CONSTRAINT_VECTOR(1), 0).transpose() * robot->Jv("link1");
				J_con[1] = CONSTRAINT_VECTOR[1].transpose() * robot->Jv("link1");  // 1 x 2
				Vector2d force_bias = 0 * Vector2d(0, 1);
				second_damping_torques = - KV * J_con[1] * robot->dq();
            ...
            if (smaller_idx == 0) {
                ...
                // task projections if second distance is active
                if (DISTANCE(1) < RHO_0(1)) {
                    J_con[1] *= robot->nullspaceMatrix(J_con[0]);
                    task_projections[1] = Sai2Model::matrixRangeBasis(J_con[1]);
                    J_con[1] = task_projections[1].transpose() * J_con[1];
                    N_obs[1] = robot->nullspaceMatrix(task_projections[1].transpose() * J_con[1]);
                    
                    // projections
                    N_obs[0] = robot->nullspaceMatrix(J_con[0]);
                    // std::cout << "1\n";
                    // std::cout << N_obs[0] << "\n";
                    // std::cout << N_obs[1] << "\n";
                    N_prec = N_obs[1] * N_obs[0];
                    // std::cout << N_prec << "\n";
                    // throw runtime_error("");
                } else {                    
                    N_prec = robot->nullspaceMatrix(J_con[0]);
                }

            } else if (smaller_idx == 1) {

                // task projections
                if (DISTANCE(0) < RHO_0(0)) {
                    J_con[0] *= robot->nullspaceMatrix(J_con[1]);
                    task_projections[0] = Sai2Model::matrixRangeBasis(J_con[0]);
                    J_con[0] = task_projections[0].transpose() * J_con[0];
                    N_obs[0] = robot->nullspaceMatrix(task_projections[0].transpose() * J_con[0]);

                    // projections
                    N_obs[1] = robot->nullspaceMatrix(J_con[1]);
                    // std::cout << "2\n";
                    // std::cout << N_obs[0] << "\n";
                    // std::cout << N_obs[1] << "\n";
                    N_prec = N_obs[0] * N_obs[1];
                    // std::cout << N_prec << "\n";
                    // throw runtime_error("");
                } else {
                    N_prec = robot->nullspaceMatrix(J_con[1]);
                    // N_prec = robot->nullspaceMatrix(J_con[0]);
                }
		...
		if (EXIT_FLAG) {
			...
			// std::cout << "constraint vector: " << CONSTRAINT_VECTOR.transpose() << "\n";
			// if (CONSTRAINT_VECTOR[0].dot(control_force.normalized()) < 0 && isInsideSuperellipse(robot->q()(0), robot->q()(1), APF_A(0), APF_B(0))) {
            if (CONSTRAINT_VECTOR[0].dot(-control_force.normalized()) < 0) {
				// N_prec = MatrixXd::Identity(dof, dof);
				first_damping_torques.setZero();
                first_apf_torque.setZero();
                N_prec = N_obs[1];
			} 

            // if (CONSTRAINT_VECTOR[1].dot(control_force.normalized()) < 0 && isInsideSuperellipse(robot->q()(0), robot->q()(1), APF_A(1), APF_B(1), APF1_CENTER(0), APF1_CENTER(1))) {
            if (CONSTRAINT_VECTOR[1].dot(-control_force.normalized()) < 0) {
				// N_prec = MatrixXd::Identity(dof, dof);
				second_damping_torques.setZero();
                second_apf_torque.setZero();
                N_prec = N_obs[0];
			} 
		}

        // // order nullspace based on distance
        // if (DISTANCE(0) < DISTANCE(1)) {
        //     N_prec = N_obs[0];
        //     N_obs[1] = N_obs[0] * N_obs[1];
        // } else {
        //     N_prec = N_obs[1];
        //     N_obs[0] = N_obs[1] * N_obs[0];
        // }

        // nullspace projection with original control
		{
			lock_guard<mutex> lock(mutex_robot);
			motion_task->updateTaskModel(N_prec);
        ...
        control_torques = VectorXd::Zero(2);
		...
		//------ compute the final torques
		{
			...
			if (time - start_time > T_WAIT) {
                ...
                // control_torques = motion_task_torques_without_nullspace;
                if (!isnan(motion_task_torques(0))) {
				    control_torques = motion_task_torques;
                }
                if (smaller_idx == 0) {
                    control_torques += 1 * MASS * first_apf_torque + 1 * J_con[0].transpose() * MASS * first_damping_torques;
                    // control_torques += robot->nullspaceMatrix(J_con[0]).transpose() * J_con[1].transpose() * task_projections[1].transpose() * (MASS * second_apf_torque + 0 * MASS * second_damping_torques);
                    control_torques += robot->nullspaceMatrix(J_con[0]).transpose() * (MASS * second_apf_torque + 0 * J_con[1].transpose() * MASS * second_damping_torques);
                    std::cout << "0: \n";
                } else if (smaller_idx == 1) {
                    // control_torques += MASS * first_apf_torque + J_con[0].transpose() * MASS * first_damping_torques;
                    control_torques += MASS * second_apf_torque * 1 + J_con[1].transpose() * MASS * second_damping_torques;
                    control_torques += robot->nullspaceMatrix(J_con[1]).transpose() * (MASS * first_apf_torque + 0 * J_con[0].transpose() * MASS * first_damping_torques);
                    std::cout << "1: \n";
                }

                // std::cout << motion_task_torques.transpose() << "\n";
                // std::cout << first_apf_torque.transpose() << "\n";
                // std::cout << second_apf_torque.transpose() << "\n";
                // std::cout << first_damping_torques.transpose() << "\n";
                // std::cout << second_damping_torques.transpose() << "\n";

                // control_torques = motion_task_torques + \
                //                     1 * N_obs[0].transpose() * (MASS * first_apf_torque + J_con[0].transpose() * task_projections[0].transpose() * MASS * first_damping_torques) + \
                //                     0 * N_obs[1].transpose() * (MASS * second_apf_torque + J_con[1].transpose() * task_projections[1].transpose() * MASS * second_damping_torques);  // not baseline 
