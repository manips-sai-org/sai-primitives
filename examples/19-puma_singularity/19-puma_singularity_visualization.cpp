...
int main(int argc, char** argv) {
    ...
    // Create two lines to form a double-sided arrow
    // chai3d::cShapeLine* arrow1 = new chai3d::cShapeLine(chai3d::cVector3d(-0.015, -0.6, 1.345), chai3d::cVector3d(-0.015, 0.6, 1.345));
    chai3d::cShapeLine* arrow1 = new chai3d::cShapeLine(chai3d::cVector3d(0.025, -0.6, 1.33), chai3d::cVector3d(0.025, 0.6, 1.33));

    // Set colors
    arrow1->m_colorPointA.setRed();
    arrow1->m_colorPointB.setRed();
    arrow1->setLineWidth(8);

    graphics->_world->addChild(arrow1);


    // add mesh plane of motion 
    // Create a mesh object
    chai3d::cMesh* plane = new chai3d::cMesh();
    chai3d::cMesh* motion_plane = new chai3d::cMesh();

    // Create a plane (size 1x1)
    chai3d::cCreatePlane(plane, 2, 2);    
    chai3d::cCreatePlane(motion_plane, 2, 2);

    // Set position and orientation
    plane->setLocalPos(chai3d::cVector3d(0, 0.15, 0.5));
    motion_plane->setLocalPos(chai3d::cVector3d(0, 0.15, 0.5));
    
    // Define a rotation (e.g., 45-degree rotation around Z-axis)
    chai3d::cMatrix3d rot;
    rot.setAxisAngleRotationDeg(0, 1, 0, 90); // Rotate 45° around Z-axis

    chai3d::cMatrix3d rot_2;
    rot_2.setAxisAngleRotationDeg(0, 0, 1, 90); // Rotate 45° around Z-axis

    // Set the rotation
    plane->setLocalRot(rot_2 * rot);
    motion_plane->setLocalRot(rot);

    // Set material properties
    plane->m_material->setGreen();
    plane->m_material->setTransparencyLevel(0.25); // 50% transparency

    motion_plane->m_material->setRed();
    motion_plane->m_material->setTransparencyLevel(0.25); // 50% transparency

    // Enable transparency rendering
    plane->setUseTransparency(true);
    motion_plane->setUseTransparency(true);
    graphics->_world->addChild(plane);
    // graphics->_world->addChild(motion_plane);
	...
	*/
    init_q(0) = 0;
	init_q(1) = - 3 * M_PI / 4;
	init_q(2) = M_PI / 2 + M_PI / 2;
	init_q(3) = M_PI / 2;
	// init_q(4) = M_PI / 4 * (0.25);
    init_q(4) = 0;
	// init_q(5) = M_PI;
