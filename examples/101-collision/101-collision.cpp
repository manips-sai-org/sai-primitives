...
int main(int argc, char** argv) {
	...
	object_init_pose.translation() = Vector3d(0.3, 0.3, 0.5);
...
void control(shared_ptr<Sai2Model::Sai2Model> robot,
	...
	while (fSimulationRunning) {
		...
		if (state == POSTURE) {
			...
			collision_handler->setObjectTransform(object_transform, 0);
			...
			//------ compute the final torques
			{
				...
				control_torques = collision_handler->computeTorques(motion_force_task_torques_without_handler + joint_task_torques_without_handler);
