	...
	while (fSimulationRunning) {
		...
		if (state == POSTURE) {
			...
			if ((robot->q() - joint_task->getGoalPosition()).norm() < 8e-2) {
				...
				joint_handler->setCollisionTime(0.15 * 1);
			...
			/*
				Joint-only method
			*/
			if (flag_joint_method) {
				// compute torques 
				{
					lock_guard<mutex> lock(mutex_torques);
					control_torques = joint_handler_constraint_torques + motion_force_torques + joint_task_torques;
					...
					// control_torques = joint_handler->computeTorques(motion_force_torques + joint_task_torques, flag_constraint_only);
					// if (flag_constraint_only) {
					// 	motion_force_task->updateTaskModel(joint_handler->getTaskAndPreviousNullspace());
					// 	joint_task->updateTaskModel(motion_force_task->getTaskAndPreviousNullspace());
					// 	control_torques += motion_force_torques + joint_task_torques;
					// }
					// control_torques = motion_force_torques + joint_task_torques;
				...
				// get the joint state 
				// if the joint is approaching the joint limit and control is active, then limit the end-effector velocity 
				// v_max is the entry velocity, v_min is a given minimum velocity >= 0
				VectorXi joint_limit_state = joint_handler->getJointLimitState();  
				VectorXd joint_limit_coeffs = joint_handler->getJointBlendingCoefficients();

				// std::cout << "Joint limit state after update model: " << joint_limit_state(2) << "\n";

				if (!flag_baseline && 
					(joint_limit_state(CONSTRAINED_JOINT_IDX) == Sai2Primitives::JointState::MIN_SOFT_POS || 
					joint_limit_state(CONSTRAINED_JOINT_IDX) == Sai2Primitives::JointState::MAX_SOFT_POS)) {

					// double joint_torque = (motion_force_torques + joint_task_torques)(2);
					// std::cout << "Joint torque on 2: " << joint_torque << "\n";
					// if ((joint_torque > 0.5 && joint_limit_state(2) == Sai2Primitives::JointState::MAX_SOFT_POS) || 
						// (joint_torque < -0.5 && joint_limit_state(2) == Sai2Primitives::JointState::MIN_SOFT_POS)) {
						double v_sat = getMaxVelFunction(std::clamp(joint_limit_coeffs(CONSTRAINED_JOINT_IDX), 0.0, 1.0), max_saturation_velocity, min_saturation_velocity);
						motion_force_task->enableVelocitySaturation(v_sat);
						std::cout << "Velocity zone limitation with saturation: " << v_sat << "\n";
						// std::cout << "Blending coefficient: " << joint_limit_coeffs(2) << "\n";
					// } else {
					...
					// send torques 
					{
						...
						if (!flag_simulation) {
							redis_client->setEigen(JOINT_TORQUES_COMMANDED_KEY, control_torques + coriolis);
						}
					}
				} else if (joint_limit_state(CONSTRAINED_JOINT_IDX) == Sai2Primitives::JointState::MIN_HARD_POS || \
						joint_limit_state(CONSTRAINED_JOINT_IDX) == Sai2Primitives::JointState::MAX_HARD_POS) {

					// std::cout << "APF\n";

					if (exit_state(CONSTRAINED_JOINT_IDX) == 0) {
						// update task model (if wanting to use the sub-space)
						// motion_force_task->updateTaskModel(joint_handler->getTaskAndPreviousNullspace());
						// joint_task->updateTaskModel(motion_force_task->getTaskAndPreviousNullspace());
					...
					locked_joint = true;
					// send torques from the joint handler (apf)
					{
						lock_guard<mutex> lock(mutex_torques);
						control_torques = \
							joint_handler->computeTorques(motion_force_torques + joint_task_torques, false, flag_baseline);

						joint_limit_state = joint_handler->getJointLimitState();
						// std::cout << "Joint limit state after compute torques: " << joint_limit_state(2) << "\n";

						if (!flag_simulation) {
							redis_client->setEigen(JOINT_TORQUES_COMMANDED_KEY, control_torques + coriolis);
						}
					}
				} else if (joint_limit_state(CONSTRAINED_JOINT_IDX) == Sai2Primitives::JointState::SAFE) {
					...
					// compute torques 
					{
						...
						// control_torques = joint_handler->computeTorques(motion_force_task_torques_without_handler + joint_task_torques_without_handler);
						// control_torques = joint_handler->computeTorques(motion_force_torques + joint_task_torques);
						control_torques = motion_force_torques + joint_task_torques + coriolis;
