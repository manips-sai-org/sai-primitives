...
public:
	...
	void setOutputGoalPosition(const Vector3d& goal_position) {
		_latest_output.robot_goal_position = goal_position;
	}

	void setOutputGoalOrientation(const Matrix3d& goal_orientation) {
		_latest_output.robot_goal_orientation = goal_orientation;
	}

	void resetRobotLinearOffset(const bool flag) {
		if (flag) {
			_reset_robot_linear_offset = true;
		}
	}

	void resetRobotAngularOffset(const bool flag) {
		if (flag) {
