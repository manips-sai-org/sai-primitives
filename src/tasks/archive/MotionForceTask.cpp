...
template <typename T> int getSign(T value) {
	static_assert(std::is_arithmetic<T>::value,
			"sign() requires an arithmetic type");
...
void MotionForceTask::initialSetup() {
	...
	_enable_zero_crossing_reset = false;
  	_prev_position_error = Vector3d::Zero();
  	_prev_step_orientation_error = Vector3d::Zero();
...
VectorXd MotionForceTask::computeTorques() {
	...
	if (_enable_zero_crossing_reset) {
	Vector3d current_position_error =
		sigma_position * (_current_position - _desired_position);
	for (int i = 0; i < 3; ++i) {
		if (getSign(current_position_error(i)) !=
			getSign(_prev_position_error(i))) {
		_integrated_position_error(i) = 0;
		}
	}
	_prev_position_error =
		sigma_position * (_current_position - _desired_position);
	_integrated_position_error += current_position_error * getLoopTimestep();
	} else {
	...
	// update integrated error for I term
	if (_enable_zero_crossing_reset) {
		Vector3d current_step_orientation_error = step_orientation_error;
		for (int i = 0; i < 3; ++i) {
			if (getSign(step_orientation_error(i)) !=
				getSign(_prev_step_orientation_error(i))) {
				_integrated_orientation_error(i) = 0;
			}
		}
		_prev_step_orientation_error = step_orientation_error;
		_integrated_orientation_error +=
		current_step_orientation_error * getLoopTimestep();
	} else {
