...
VectorXd JointHandler::computeTorques(const VectorXd& torques) {
    ...
    if (_num_con == 0 || !_enable_limit_flag) {
        ...
        if (_enable_vel_limits) {
            ...
            // decomposition 
            MatrixXd current_task_range = Sai2Model::matrixRangeBasis(projected_jacobian);
            Sai2Model::OpSpaceMatrices con_matrices = _robot->operationalSpaceMatrices(current_task_range.transpose() * projected_jacobian);
