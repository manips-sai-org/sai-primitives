...
bool SingularityHandler::classifySingularityType(const std::vector<MatrixXd>& kinematic_hessian,
    ...
    // VectorXd dsdq(_robot->dof());
    // for (int i = 0; i < kinematic_hessian.size(); ++i) {
    //     dsdq(i) = u.transpose() * kinematic_hessian[i] * v;
    // } 
    // double deviation = dsdq.norm();
...

    {
        ...
        std::cout << "2nd order deviation vector: " << getSecondOrderExpansion(kinematic_hessian, v).transpose() << "\n";
    }
    ...
    const MatrixXd& V)
{
    ...
    /* ---------------------------------------------
     * 1. Precompute Vᵀ H_i V   (i = task dimension)
     * --------------------------------------------- */
    ...
    {
        // debug 
        std::cout << "initial solver eigenvectors: \n" << _eig_solver.eigenvalues() << "\n";
        std::cout << "initial deviations: " << deviation.transpose() << "\n";
    }
...
// VectorXd SingularityHandler::getInitialVector(const std::vector<MatrixXd>& kinematic_hessian,
//                                               const MatrixXd& U,
//                                               const MatrixXd& V) {

//     // compute vector of matrices H_i (one index for each task dimension)
//     std::vector<MatrixXd> H_dof(static_cast<int>(U.rows()), MatrixXd::Zero(_robot->dof(), _robot->dof()));
...
//     // compute A for weighted matrices
//     std::vector<MatrixXd> A_weighted(static_cast<int>(U.cols()), MatrixXd::Zero(U.cols(), U.cols()));
//     for (int i = 0; i < U.rows(); ++i) {
//         for (int j = 0; j < U.cols(); ++j) {
//             A_weighted[j].noalias() += V.transpose() * U(i, j) * H_dof[i] * V;
//         }
//     }
//     // compute sum squared hessian
//     MatrixXd H_sum_squared = MatrixXd::Zero(A_weighted[0].cols(), A_weighted[0].cols());
//     for (int i = 0; i < A_weighted.size(); ++i) {
//         H_sum_squared.noalias() += A_weighted[i] * A_weighted[i];
//     }

//     // compute eigendecomposition
//     _eig_solver.compute(H_sum_squared);

//     VectorXd deviation(U.cols());
//     for (int i = 0; i < U.cols(); ++i) {
//         // evalute
//         VectorXd u = (U * _eig_solver.eigenvectors().col(i)).normalized();
//         VectorXd v = (V * _eig_solver.eigenvectors().col(i)).normalized();
//         deviation(i) = std::abs(v.dot(getProjectedHessian(kinematic_hessian, u) * v));
//     }

//     {
//         // debug
//         std::cout << "initial guess function deviation: " << deviation.transpose() << "\n";
//         std::cout << "eigenvalues: " << _eig_solver.eigenvalues().transpose() << "\n";
//         std::cout << "eigenvectors: \n" << _eig_solver.eigenvectors() << "\n";
//     }

//     Eigen::Index min_index;
//     deviation.minCoeff(&min_index);
//     return _eig_solver.eigenvectors().col(static_cast<int>(min_index));  // coefficient vector 

// }

bool SingularityHandler::checkBasisForType1(const std::vector<MatrixXd>& kinematic_hessian,
                                                                 const MatrixXd& U,
                                                                 const MatrixXd& V) {

    // general approach with eigenvalue computation of the un-projected 2nd order deviations 
    ...
    std::vector<MatrixXd> H_weighted = {2, MatrixXd::Zero(V.cols(), V.cols())};
    for (int i = 0; i < U.rows(); ++i) {
        for (int j = 0; j < U.cols(); ++j) {
            H_weighted[j].noalias() += U(i, j) * H_dof[i];
            // H_weighted[j].noalias() += 1 * H_dof[i];
        }
    }

    // compute lower bound 
    VectorXd lambda_min(H_dof.size());
    for (int i = 0; i < H_dof.size(); ++i) {
        _eig_solver.compute(H_dof[i] * H_dof[i]);
        lambda_min(i) = _eig_solver.eigenvalues().minCoeff();
        

        // {
        //     // debug
        //     std::cout << "matrix: \n" << H_dof[i] << "\n";
        //     std::cout << "evalues: \n" << _eig_solver.eigenvalues().transpose() << "\n";
        //     std::cout << "evectors: \n" << _eig_solver.eigenvectors() << "\n";
        // }
    }

    double lower_bound = lambda_min.minCoeff();
...
//     if (self->_nl_opt_data->flag_type_1_search) {
//         // double deviation = - std::abs(u.transpose() * self->getSecondOrderExpansion(self->_dJdq, v));
//         double deviation = - std::abs(v.dot(self->getProjectedHessian(self->_dJdq, u) * v));
//         return deviation;
//     } else {
//         // double deviation = std::abs(u.transpose() * self->getSecondOrderExpansion(self->_dJdq, v));    
//         double deviation = std::abs(v.dot(self->getProjectedHessian(self->_dJdq, u) * v));
...
double SingularityHandler::objective(const std::vector<double>& x,
                                     ...
                                     void* f_data)
{
    ...
    // Safety epsilon to prevent division by zero at the origin
    const double safe_V_norm = (V_norm < 1e-12) ? 1e-12 : V_norm;
    const VectorXd v = V / safe_V_norm;

    /* -------------------------------------------------
     * 2. Build u direction (The "Target" direction)
     * ------------------------------------------------- */
    ...
    if (data.flag_zero_value) {
        ...
        u.head<3>() = self->_robot->position(self->_link_name,
                                            self->_compliant_frame.translation())
                      - data.starting_position;

        u.tail<3>() = Sai2Model::orientationError(
                        self->_robot->rotation(self->_link_name,
                                              self->_compliant_frame.linear()),
                        data.starting_orientation);

        // Project onto singular task range and re-normalize
        u = data.singular_task_range * (data.singular_task_range.transpose() * u);
        u.normalize();
    }

    /* -------------------------------------------------
     * 3. Compute Value: f(x) = +/- | v^T * M * v |
     * ------------------------------------------------- */
    const MatrixXd M = self->getProjectedHessian(self->_dJdq, u);
    const VectorXd Mv = M * v;
    const double quadratic_form = v.dot(Mv);
    
    // const double abs_val = std::abs(quadratic_form);
    const double abs_val = quadratic_form * quadratic_form;
    const double result = data.flag_type_1_search ? -abs_val : abs_val;

    /* -------------------------------------------------
     * 4. Compute Gradient (Analytical)
     * ------------------------------------------------- */
    if (!grad.empty()) {
        // 4a. Handle the derivative of the absolute value and search type
        // d/dx [+/- |g(x)|] = (+/-) * sgn(g(x)) * dg/dx
        double sgn_g = (quadratic_form > 0) ? 1.0 : (quadratic_form < 0 ? -1.0 : 0.0);
        double total_multiplier = (data.flag_type_1_search ? -1.0 : 1.0) * sgn_g;

        // 4b. Loop through variables
        // The gradient of v w.r.t x_i is the projection of basis_i onto 
        // the tangent plane of the unit sphere.
        for (int i = 0; i < n; ++i) {
            const VectorXd& Bi = data.basis[i];
            
            // Derivative of normalized vector v w.r.t x_i
            // dv/dxi = (1/|V|) * (Bi - v * (v . Bi))
            VectorXd dv_dxi = (Bi - v * v.dot(Bi)) / safe_V_norm;

            // Chain rule: dg/dxi = 2 * (M*v)^T * (dv/dxi)
            // Note: We treat M (and thus u) as locally constant w.r.t x for the gradient
            double dg_dxi = 2.0 * Mv.dot(dv_dxi);

            grad[i] = total_multiplier * dg_dxi;
        }
    }
...
//     /* -------------------------------------------------
//      * 1. Build v = sum x_i * basis_i   (no temp allocs)
//      * ------------------------------------------------- */
//     VectorXd v = VectorXd::Zero(self->_dof);
//     const int n = static_cast<int>(x.size());
//     for (int i = 0; i < n; ++i) {
...
//     if (data.flag_zero_value) {
//         // Forward kinematics only when needed
//         self->_robot->setQ(data.starting_q + data.perturb_step_size * v);
//         self->_robot->updateKinematics();

//         u.head<3>() =
//             self->_robot->position(self->_link_name,
//                                    self->_compliant_frame.translation())
//             - data.starting_position;
...
//         u = data.singular_task_range *
//             (data.singular_task_range.transpose() * u);
//         u.normalize();
//     }

//     /* -------------------------------------------------
//      * 3. Shared projected Hessian evaluation
//      * ------------------------------------------------- */
//     const double val =
//         v.dot(self->getProjectedHessian(self->_dJdq, u) * v);

//     /* -------------------------------------------------
//      * 4. Sign handling
//      * ------------------------------------------------- */
//     return data.flag_type_1_search ? -std::abs(val) : std::abs(val);
// }

std::pair<VectorXd, VectorXd>
SingularityHandler::getTowardSingularityDirection(
...
{
    VectorXd u_toward = u;
    const Vector3d u_pos = u.head<3>();

    auto evalDeviation = [&](const VectorXd& q_sample,
                             Vector3d& delta_out) -> double {
        _robot->setQ(q_sample);
        _robot->updateKinematics();
        delta_out =
            _robot->position(_link_name,
                             _compliant_frame.translation()) - curr_pos;
        return u_pos.dot(delta_out);
    };

    Vector3d pos_delta, neg_delta;
    ...
    for (int cnt = 0; cnt <= _type_1_search_max_iter; ++cnt) {
        ...
        // --------------------------------------------------
        // Case 1: same sign → both retract or both extend
        // --------------------------------------------------
        if (sign(pos_dev) == sign(neg_dev)) {

            // If motion is extending, flip u
            if (pos_dev > 0.0) {
                u_toward = -u_toward;
            }

            // Determine correct dsdq sign with ONE more FK
            Vector3d test_delta;
            evalDeviation(curr_q - step_size * dsdq, test_delta);

            const bool flip_dsdq =
                u_toward.head<3>().dot(test_delta) < 0.0;
    ...
    // --------------------------------------------------
    // Fallback: pick stronger retracting direction
    // --------------------------------------------------
                                       ...
                                       _verbose(verbose)
{
    ...
    for (int i = 2; i < 7; ++i) {
        ...
        // _nl_opt[i] = std::make_unique<nlopt::opt>(nlopt::LD_SLSQP, i);
        ...
        // _nl_opt[i]->set_max_objective(epigraph_objective, this);
        // _nl_opt[i]->add_equality_constraint(equality, this, 1e-2);
        // _nl_opt[i]->add_inequality_constraint(upper_epigraph, this, 1e-2);
        // _nl_opt[i]->add_inequality_constraint(lower_epigraph, this, 1e-2);
...
void SingularityHandler::classifySingularity(const MatrixXd& projected_jacobian,
    ...
    if (_degenerate_indices.size() > 0) {
        ...
        for (auto group : _degenerate_indices) {
            ...
            {
                // debug
                // auto _flag = checkBasisForType1(_dJdq, curr_singular_task_range, curr_singular_joint_task_range);
            }
            ...
            while (true) {
                ...
                // initial guess from smallest eigenvalue
                ...
                {
                    // debug
                    std::cout << "nm initial coeff: " << sjs_coefficients << "\n";
                }
                ...
                {
                    // debug 
                    VectorXd _u_test = vectorFromBasis(sjs_coefficients, curr_singular_task_range);
                    VectorXd _v_test = vectorFromBasis(sjs_coefficients, curr_singular_joint_task_range);
                    std::cout << "nm optimal value: " << sqrt(optimal_value) << "\n";
                    std::cout << "nm optimal coefficients: " << sjs_coefficients << "\n";
                    std::cout << "nm u: " << _u_test.transpose() << "\n";
                    std::cout << "nm v: " << _v_test.transpose() << "\n";
                    std::cout << "re-computed deviation: " << _u_test.transpose() * getSecondOrderExpansion(_dJdq, _v_test) << "\n";
                }
                ...
                if (sqrt(optimal_value) > _type_1_tol) {
                    // set as type 1 singularity and switch
                    search_type_1 = true;
            ...
            */
                ...
                for (int i = 0; i < curr_type_1_singular_task_range.size(); ++i) {
                    ...
                    if (_degenerate_singular_values[group_id].maxCoeff() < _type_1_search_tol) {
                        ...
                        // delta_vector.tail(3) = Sai2Model::orientationError(_robot->rotation(_link_name, _compliant_frame.linear()), curr_ori);
            ...
            for (int i = 0; i < curr_type_2_singular_task_range.size(); ++i) {
                ...
                _active_singularities.push_back(Singularity(curr_type_2_singular_task_range[i], 
    ...
    if (!_is_in_singularity || !_enforce_handling_strategy) {
        ...
        for (auto ind : sorted_indices) {
            ...
            MatrixXd curr_lambda = 
                Sai2Model::computePseudoInverse(curr_projected_jacobian * _robot->MInv() * curr_projected_jacobian.transpose(), _type_1_search_tol);
            ...
            VectorXd full_acceleration_component = 
                (_robot->MInv() * (_non_singular_task_torques + saturateBox(singular_torque_component, _tau_lower, _tau_upper))).normalized();
            // VectorXd singular_unit_acceleration_component = 
            //     (_robot->MInv() * saturateBox(singular_torque_component, _tau_lower, _tau_upper)).normalized();
            VectorXd singular_unit_acceleration_component = _robot->MInv() * singular_torque_component;

            // method going to 1 dof space 
            double vs_force = (curr_projected_jacobian * singular_unit_acceleration_component)(0);

            // get the directionality of vs that moves away from singularity
            int vs_sign_away_from_singularity = 1;
            if (_active_singularities[ind].dsdq.dot(_active_singularities[ind].v) < 0) {
                vs_sign_away_from_singularity = -1;
            }
            ...
            if (_active_singularities[ind].type == TYPE_1_SINGULARITY) {
                ...
                // /*
                //    Test with continuity with Type 1 strategy
                // */
                // // joint-space classification of control towards/away from singularity
                // bool is_moving_towards_singularity;
                // if (singular_unit_acceleration_component.normalized().dot(_active_singularities[ind].dsdq) > 0) {
                // // if (sign(vs_force) == sign(vs_sign_away_from_singularity)) {
                //     is_moving_towards_singularity = false;
                // } else {
                //     is_moving_towards_singularity = true;
                // }
                ...
                double weighted_force_dotted_singular_direction = 
                    std::clamp((1 - exp(-_type_2_vel_scheduling * force_dotted_singular_direction)) / (1 - exp(-_type_2_vel_scheduling)), 0.0, 1.0);
      
                // VectorXd dq_des = _active_singularities[ind].v * _active_singularities[ind].v.transpose() * singular_unit_acceleration_component.normalized();
                // VectorXd dq_des = singular_unit_acceleration_component;

                // // test with continuity with type 1 
                // if (is_moving_towards_singularity) {
                //     // match dq_des sign with -dsdq
                //     if (dq_des.dot(_active_singularities[ind].dsdq) > 0) {
                //         dq_des *= -1;
                //     }
                //     // dq_des = - _active_singularities[ind].dsdq;
                // } else {
                //     // match dq_des sign with dsdq
                //     if (dq_des.dot(_active_singularities[ind].dsdq) < 0) {
                //         dq_des *= -1;
                ...
                VectorXd dq_des = _type_2_max_vel * weighted_force_dotted_singular_direction * singular_unit_acceleration_component.normalized();
                ...
                //     std::cout << "type 2 unit torque: " << unit_torques.transpose() << "\n";
                    std::cout << "force dotted singular direction: " << force_dotted_singular_direction << "\n";
